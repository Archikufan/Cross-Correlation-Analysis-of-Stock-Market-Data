...
def centroid_function(y_values, time_lags):
    ...
    differences = intersection_indices - peak_index
    # print(differences)
    ...
    closest_indices = np.array([peak_index+negative_diff[-1],peak_index+positive_diff[0]])
    # print(closest_indices)
    # Get the corresponding values
    closest_values = [time_lags[i] for i in closest_indices]
    centroid = np.mean(closest_values)
    # print(closest_values)
    
    # plt.axvline(x = closest_values[0], color = "r")
    # plt.axvline(x = closest_values[1], color = "r")
...
def centroid_stdv(delay,p1,d1,p2,d2):
    ...
    for i in range(1000):
        ...
        # plt.axhline(y=0.8*min(iccfs), linestyle='--', label='0.8*Peak')
...
def anti_corr_centroid(y_values, time_lags):
    ...
    # Find the threshold value (80% of the peak)
    threshold1 = 0.70 * peak_value
    threshold2 = 0.95* peak_value
    # Find the indices where the ICCF intersects the threshold
    condition = np.logical_and(y_values <= threshold1, y_values >= threshold2)
    intersection_indices = np.where(condition)[0]
    print(intersection_indices)
...
#anit_corr_tau = anti_corr_centroid(iccfs,tau)

#sm_anit_corr_tau = anti_corr_centroid(smoothed_iccfs,tau)
...
plt.axvline(x = smoothed_tau[1], color = "g")
plt.axvline(x = smoothed_tau[0], color = "purple")
#plt.axvline(x = anit_corr_tau[1])
plt.xlabel("Time Lag [Days]");plt.ylabel("ICCF Value");plt.title("ICCF of "+ file1 + " and "+file2);
#plt.xlim(-100,200)
plt.show()
...
# print(f"For the smoothed data\n Anti Correlation Tau Peak = {(sm_anit_corr_tau[0])} ± {round(sm_tau_stdv[0],2)}\
#           \n Anti Correlation Tau Centroid = {sm_anit_corr_tau[1]} ± {round(sm_tau_stdv[1],2)}")
